...
int main(void) {
	...
	NinjaTrap Gloug("Gloug");
	ClapTrap Amy("Amy");

	Gloug.ninjaShoebox(John);
	Gloug.ninjaShoebox(Donald);
	Gloug.ninjaShoebox(Gloug);
	Gloug.ninjaShoebox(Amy);

	Jack.takeDamage(Gloug.meleeAttack(Jack.get_name()));
	Gloug.takeDamage(John.rangedAttack(Gloug.get_name()));
	human.takeDamage(Gloug.rangedAttack(human.get_name()));
	...
	std::cout << Donald.get_name() << " has " << Donald.get_hit() << " health left." << std::endl;
	std::cout << Kwame.get_name() << " has " << Kwame.get_hit() << " health left." << std::endl;
	std::cout << Gloug.get_name() << " has " << Gloug.get_hit() << " health left." << std::endl;
	std::cout << Amy.get_name() << " has " << Amy.get_hit() << " health left." << std::endl;
